...
public:
    ...
    vector<bool> to_bool_vec(const map<int, int>& group) const {
        vector<bool> bool_vec;
        assert(group.size() == ass.size());
        bool_vec.resize(group.size());
        for (auto p : ass) {
            bool_vec[group.at(p.first)] = p.second;
        }
        return bool_vec;
    }
...
Cluster::Cluster(map<int, int>&& group, const vector<vector<bool>>&& assignments_) : var_to_idx(group), assignment(assignments_) {}

Cluster get_cluster(int init_var, const vtc_T& var_to_clauses, const vector<vector<int>>& clause_list, int max_count, const set<int>& used) {
    ...
    vector<vector<bool>> bool_vec_assignments;
    for (const auto& ass : assignments) {
        bool_vec_assignments.push_back(ass.to_bool_vec(group));
    }

    assignments.clear(); // have to make sure implication trackers dtor gets called before implication_counter goes away
    return {move(group), move(bool_vec_assignments)};

    /*

    compute_skyline(assignments, var_to_clauses, clause_list);
    //assignments.clear(); // have to make sure implication trackers dtor gets called before implication_counter goes away
    //return group;

    set<int> relavent_clauses;
    for (auto p : group) {
        for (int clause_num : var_to_clauses.at(p.first).contains_default) {
            relavent_clauses.insert(clause_num);
        }
        for (int clause_num : var_to_clauses.at(p.first).contains_neg) {
            relavent_clauses.insert(clause_num);
        }
    }
    ...
    for (int i = 0; i < assignments.size(); ++i) {
        ...
        for (int j = i + 1; j < assignments.size(); ++j) {
            ...
            for (int var : intersection) {
                for (int clause_num : var_to_clauses.at(var).contains_default) {
                    relavent_clauses_with_intersection.insert(clause_num);
                }

                for (int clause_num : var_to_clauses.at(var).contains_neg) {
                    relavent_clauses_with_intersection.insert(clause_num);
                }
            }
